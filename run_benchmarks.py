...
def get_scc_filtration_size(file_path: str | Path) -> int:
    # Count simplices, subtract header lines (4)
...
def benchmark(X: np.ndarray, k_max: int, delete_temp: bool=True) -> dict:
    ...
    t_start = time.time()
    subprocess.run(["rhomboidtiling", str(input_file), str(output_file_sliced), str(X.shape[1]), str(k_max), "firep", str(homology_dimension)], capture_output=True, text=True)
    t_end = time.time()
    rhomboid_time = t_end - t_start
    rhomboid_size = get_scc_filtration_size(output_file_sliced)
    ...
    t_start = time.time()
    subprocess.run(["rhomboidtiling", str(input_file), str(output_file_unsliced), str(X.shape[1]), str(k_max), "ufirep", str(homology_dimension)], capture_output=True, text=True)
    t_end = time.time()
    rhomboid_time = t_end - t_start
    rhomboid_size = get_scc_filtration_size(output_file_unsliced)
